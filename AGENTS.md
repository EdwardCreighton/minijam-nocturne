...
- Menu (P7): `Menu/MainMenuController.cs` + `CreditsPanel.cs`; `MainMenu.unity` has `MenuRoot` (title/buttons, hidden under credits), `CreditsPanel`, overlay Canvas, UI-wired `EventSystem`, plus an `Audio` GO (`AudioManager`: `Click Sound` + looping `musicTrack` with `playMusicOnStart`, started in `Start`), plus a topmost `FadeOverlay` (`Image` black alpha 0 + `UI/ScreenFader.cs`). `New Game` runs `TransitionToLevel` (click → parallel `FadeOut` + `StopMusic` over `transitionFade`, unscaled → load level; fresh `RunState` by construction).
- Level screens + audio + tests (P8): `UI/Screens.cs` (Briefing/Death/Win/Pause via `GameState.StateChanged`), `FinishPoint` trigger → `OnFinishReached` (stats + menu/new-game), `Core/AudioManager.cs` (gameplay sounds still stub `[Audio]` calls; real `Click Sound` clip for UI — all calls via null-safe statics `Click()`/`ClickThenLoad()`/`Swing()`/`Hit()`/`GateOpen()`/`Death()`/`Win()`, never `if (Instance)` copies), `UI/UiClickSound.cs` (drop-on-Button click sound, never combine with explicit `Click()` calls on the same button), `Hud` HP row, `Tests/EditMode` (RunState/Difficulty/defaults/gate-ids) + `Tests/PlayMode` (death persist, hold open/cancel, pause, win; unscaled waits only).
...
- UI audio: plain buttons call `AudioManager.Click()`; scene-transition buttons call `AudioManager.ClickThenLoad()` — sync `LoadScene` destroys the `AudioSource` at end of frame and cuts `PlayOneShot`, so the load waits `clickSound.length + 0.05s` realtime by default (explicit delay still possible; transitions run at `timeScale = 0`, `WaitForSecondsRealtime` only; double-clicks during the wait are ignored via `transitionPending` with `try/finally` reset). `AudioListener.pause` is on during Briefing/Pause/Win, so the shared source uses `ignoreListenerPause`. The `Click Sound` clip is assigned per scene (menu `Audio` GO + level `GameSystems`).
