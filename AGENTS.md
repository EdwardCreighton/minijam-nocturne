...
- Menu (P7): `Menu/MainMenuController.cs` + `CreditsPanel.cs`; `MainMenu.unity` has `MenuRoot` (title/buttons, hidden under credits), `CreditsPanel`, overlay Canvas, UI-wired `EventSystem`, plus an `Audio` GO (`AudioManager`, Click Sound assigned per scene). `New Game` loads the level via `AudioManager.ClickThenLoad` (fresh `RunState` by construction).
